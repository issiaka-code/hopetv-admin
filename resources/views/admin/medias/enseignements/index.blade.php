            ...
            <div class="col-12">
                <div class="d-flex justify-content-between align-items-center section-header">
                    ...
                    data-target="#addstoreModal"
                     data-route="{{ route('enseignements.store') }}" 
        ...
        <div class="row mb-4">
            <form method="GET" action="{{ route('enseignements.index') }}" class="w-100">
                <div class="row g-2 d-flex flex-row justify-content-end align-items-center">
                    <!-- Champ recherche -->
                    <div class="col-3">
                        <input type="text" name="search" value="{{ request('search') }}" class="form-control" placeholder="Rechercher un enseignement...">
                    </div>
                    ...
                    <div class="enseignement-grid-item">
                        <div class="card enseignement-card">
                            <div class="enseignement-thumbnail-container">
                                <div class="enseignement-thumbnail position-relative" data-media-id="{{ $id }}" >
                            ...
                            <div class="card-body">
                                ...
                                <div class="d-flex justify-content-between align-items-center flex-wrap">
                                    ...
                                    <div class="btn-group">
                                        <button class="btn btn-sm btn-outline-info view-enseignement-btn rounded" title="Voir l'enseignement" data-media-id="{{ $id }}">
                                            <i class="fas fa-eye"></i>
                                        </button>
                                        <button class="btn btn-sm btn-outline-primary edit-enseignement-btn mx-1 rounded" title="Modifier l'enseignement" data-media-id="{{ $id }}">
                                            <i class="fas fa-edit"></i>
                                        </button>

                                        <form action="{{ route('enseignements.destroy', $id) }}" method="POST" class="d-inline delete-form">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="btn btn-sm btn-outline-danger rounded" title="Supprimer l'enseignement" onclick="return confirm('Êtes-vous sûr de vouloir supprimer cet enseignement ?')">
                                        ...
                                        @if (in_array($media_type, ['video_link', 'video_file']))
                                        <button class="btn btn-sm btn-outline-{{ $is_published ? 'success' : 'secondary' }} toggle-publish-btn mx-1 rounded" title="{{ $is_published ? 'Dépublier' : 'Publier' }} la vidéo" data-enseignement-id="{{ $id }}" data-status="{{ $is_published ? 1 : 0 }}">
...
<script>
    $(document).ready(function() {
        ...
        $(document).on('click', '.edit-enseignement-btn', function() {
            handleEditMedia(
                $(this)
                , "{{ route('enseignements.edit', ':id') }}"
                , "{{ route('enseignements.update', ':id') }}"
                , '#editModal'
            );
        });
    
        $(document).on('click', '.view-enseignement-btn, .enseignement-thumbnail', function() {
             handleMediaView($(this), "{{ route('enseignements.voir', ':id')  }}");
        });
