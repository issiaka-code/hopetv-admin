        ...
        <div class="row mb-4">
            <form method="GET" action="{{ route('podcasts.index') }}" class="w-100">
                <div class="row g-2 d-flex flex-row justify-content-end align-items-center">
                    <!-- Champ recherche -->
                    <div class="col-3">
                        <input type="text" name="search" value="{{ request('search') }}" class="form-control" placeholder="Rechercher un podcast...">
                    </div>
                            ...
                            <div class="card-body">
                                ...
                                <div class="d-flex justify-content-between align-items-center flex-wrap">
                                    ...
                                    <div class="btn-group">
                                        ...
                                        <form action="{{ route('podcasts.destroy', $podcast->id) }}" method="POST" class="d-inline delete-form">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="btn btn-sm btn-outline-danger rounded" title="Supprimer le podcast" onclick="return confirm('Êtes-vous sûr de vouloir supprimer ce podcast ?')">
    ...
    $(document).ready(function() {
        ...
        $(document).on('click', '.toggle-publish-podcast-btn', function() {
            ...
            $.post(url, {
                ...
                .fail(function() {
                    alert('Erreur lors du changement de statut du podcast');
                });
        });
        ...
        $('input[name="media_type"]', '#addPodcastForm').change(function() {
            ...
            if (selectedType === 'audio') {
                ...
                $('#addAudioThumbnaillink').attr('required', 'required');
                // Note: addLinkThumbnail n'est pas requis pour les liens vidéo
            }
        });

        // Gestion des labels de fichiers
        $('#addAudioFile, #addVideoFile, #addAudioThumbnail, #addVideoThumbnail, #addLinkThumbnail,#addAudioThumbnaillink', ).on(
            'change'
            , function() {
        ...
        $('input[name="media_type"]', '#editPodcastForm').change(function() {
            ...
            $('#editAudioThumbnail, #editVideoThumbnail', '#editAudioThumbnaillink').removeAttr('required');
